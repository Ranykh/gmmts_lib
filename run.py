...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_gating_dataset', type=bool, default=False, help='save gating dataset')
    ...
    # Determine if this is a text model (tsft) or numerical model (tsfn)
    if hasattr(args, 'llm_model') and args.llm_model and args.prompt_weight == 1.0:
        model_type = 'tsft'
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            if args.save_gating_dataset:
                gating_setting = setting = '{}_{}_{}_{}_{}_ft{}_sl{}_ll{}_pl{}_dm{}_nh{}_el{}_dl{}_df{}_expand{}_dc{}_fc{}_eb{}_dt{}_{}_{}'.format(
                args.task_name,
                model_type,
                args.model_id,
                model_name,
                ...
                args.distil,
                args.des, ii)
                if os.path.exists('./test_gating_dataset/'+ gating_setting + '/'):
                    print('>>>>>>>Already trained and tested for gating dataset generation : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
                    continue
            ...
            now_mse=exp.test(setting, test=0, data_flag='test')
            ...
            if args.save_gating_dataset:
                ...
                print('>>>>>>>Saving Train Gating Dataset<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<')
                now_mse=exp.test(setting, data_flag='train', save_gating_dataset=True)
                
            
                print('>>>>>>>Saving Validation Gating Dataset<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<')
                now_mse=exp.test(setting, data_flag='val', save_gating_dataset=True)
                
            
                print('>>>>>>>Saving Test Gating Dataset<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<')
                now_mse=exp.test(setting, data_flag='test', save_gating_dataset=True)
                torch.cuda.empty_cache()
    else:
        print("mode not supported.. existing")
