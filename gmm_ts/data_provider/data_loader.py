...
from gmm_ts.utils.timefeatures import time_features
from gmm_ts.data_provider.m4 import M4Dataset, M4Meta
from gmm_ts.data_provider.uea import subsample, interpolate_missing, Normalizer
#from sktime.datasets import load_from_tsfile_to_dataframe
import warnings
from gmm_ts.utils.augmentation import run_augmentation_single
from os.path import join

warnings.filterwarnings('ignore')

class GatingDataset(Dataset):
    def __init__(self, experts_config, flag):
        
        self.experts = {}
        # read json file
          
        # load raw inputs, gt Y, predicted Y, numeric and textual expert latents
        for i, (expert, cfg) in enumerate(experts_config.items()):
            
            folder_path = join(flag + "_gating_dataset", cfg["folder_path"])
            if not os.path.exists(folder_path):
                raise FileNotFoundError(f"Folder path {folder_path} does not exist.")
            if i == 0:
                self.y_true = np.load(join(folder_path,'true.npy'))
                self.x_n = torch.stack(torch.load(join(folder_path,'raw_inputs.pth'), map_location="cpu"))
                self.x_n = self.x_n.reshape(self.x_n.shape[0]*self.x_n.shape[1], -1)
                if len(self.x_n.shape) == 2:
                    self.x_n = self.x_n.unsqueeze(-1)
                print(flag)
                print("y shape:{}".format(self.y_true.shape))
                print("x shape:{}".format(self.x_n.shape))
            
            expert_dict = {}
            expert_dict["pred_y"] = np.load(join(folder_path,'pred.npy'))
            latent_num_embs_path = join(folder_path,'latent_num_embs.pth')
            if os.path.exists(latent_num_embs_path):
                h = torch.stack(torch.load(latent_num_embs_path, map_location="cpu"))
                expert_dict["h_n"] = h.reshape(h.shape[0]*h.shape[1], -1)
                print(expert)
                print("latent num shape:{}".format(expert_dict["h_n"].shape))
            latent_txt_embs_path = join(folder_path,'latent_text_embs.pth')
            if os.path.exists(latent_txt_embs_path):
                #h = torch.stack(torch.load(latent_txt_embs_path, map_location="cpu"))
                #expert_dict["h_t"] = h.reshape(h.shape[0]*h.shape[1], -1)
                ...
                expert_dict["h_t"] = h
                print(expert)
                print("latent txt shape:{}".format(expert_dict["h_t"].shape))
            self.experts[expert] = expert_dict
    ...
    def __getitem__(self, idx):
        ...
        sample = {"y_true": self.y_true[idx],
                  "x_n": self.x_n[idx]}
        for expert, expert_dict in self.experts.items():
            sample[expert+"_pred_y"] = expert_dict["pred_y"][idx]
            if "h_n" in expert_dict:
                sample[expert+ "_h_n"] = expert_dict["h_n"][idx]
            if "h_t" in expert_dict:
                sample[expert+ "_h_t"] = expert_dict["h_t"][idx]
