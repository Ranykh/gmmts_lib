from gmm_ts.data_provider.data_factory import data_provider
from gmm_ts.exp.exp_basic import Exp_Basic
...
import warnings
import numpy as np
from gmm_ts.utils.dtw_metric import dtw,accelerated_dtw
from gmm_ts.utils.augmentation import run_augmentation,run_augmentation_single
import pandas as pd
...
class MLP(nn.Module):
    ...
    def forward(self, x): # LLM embedding of generated predictions 
        for i, layer in enumerate(self.layers):
            ...
            if i < len(self.layers) - 1:  
                ...
                if i == len(self.layers)-2:
                    h = x
        return x, h # time series prediction, latent embedding
warnings.filterwarnings('ignore')



class Exp_Online_Gating_Long_Term_Forecast(Exp_Basic):
    def __init__(self, args):
        # Initialize args and model_dict first (from Exp_Basic)
        self.args = args
        self.model_dict = {
            'TimesNet': TimesNet,
        ...
        # Don't call super().__init__() because it expects _build_model() to return a single model
        # but we return a list. We'll manually build models after setting up our attributes.
        configs=args
        ...
        # Build models first (returns list) and move each to device
        # This sets self.model_names which is needed for gating module
    ...
    def _build_model(self):
        self.model_names = self.args.model.split(',')
        model = [self.model_dict[model_name].Model(self.args).float() for model_name in self.model_names]

        if self.args.use_multi_gpu and self.args.use_gpu:
            model = [nn.DataParallel(m, device_ids=self.args.device_ids) for m in model]
        return model
    ...
    def _select_optimizer(self):
        model_optim = [optim.Adam(m.parameters(), lr=self.args.learning_rate) for m in self.model]
        return model_optim
    ...
    def _select_optimizer_weight(self):
        ...
        return model_optim
    def _select_optimizer_gating(self):
        model_optim = optim.Adam(self.gating_module.parameters(), lr=self.args.learning_rate_weight)
        return model_optim
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(vali_loader):
                ...
                prompt_emb, latent_text_emb = self.mlp(prompt_emb)  # (batch, prompt_token, text_embedding_dim)
                # decoder input
    ...
    def train(self, setting):
        ...
        path = [os.path.join('./online_gating_checkpoints/model/', os.path.join(setting, name)) for name in self.model_names]
        for p in path:
            if not os.path.exists(p):
                os.makedirs(p)
       
        mlp_path = os.path.join('./online_gating_checkpoints/mlp/', setting)
        if not os.path.exists(mlp_path):
            os.makedirs(mlp_path)
        gating_model_path = os.path.join('./online_gating_checkpoints/gating/', setting)
        if not os.path.exists(gating_model_path):
            os.makedirs(gating_model_path)
        ...
        early_stopping = [EarlyStopping(patience=self.args.patience, verbose=True) for _ in range(len(self.model))]
        early_stopping_mlp = EarlyStopping(patience=self.args.patience, verbose=True)
        early_stopping_gating = EarlyStopping(patience=self.args.patience, verbose=True)

        model_optim = self._select_optimizer()
        model_optim_mlp = self._select_optimizer_mlp()
        model_optim_proj = self._select_optimizer_proj()
        model_optim_gating = self._select_optimizer_gating()
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(train_loader):
                iter_count += 1
                _= [mo.zero_grad() for mo in model_optim]
                model_optim_mlp.zero_grad()
                model_optim_proj.zero_grad()
                model_optim_gating.zero_grad()
                batch_x = batch_x.float().to(self.device)
                ...
                if self.args.use_amp:
                    ...
                    loss.backward()
                    [mo.step() for mo in model_optim]
                    model_optim_mlp.step()
                    model_optim_proj.step()
                    model_optim_gating.step()
            ...
            _ = [early_stopping[i](vali_loss, self.model[i], path[i]) for i in range(len(early_stopping))]
            early_stopping_mlp(vali_loss, self.mlp, mlp_path)
            early_stopping_gating(vali_loss, self.gating_module, gating_model_path)
            do_early_stop = False
            for i in range(len(self.model)):
                if early_stopping[i].early_stop:
                    do_early_stop = True
                    break
            if do_early_stop:
        ...
        for i in range(len(self.model)):
            best_model_path = path[i] + '/' + 'checkpoint.pth'
            self.model[i].load_state_dict(torch.load(best_model_path))
        
        best_mlp_path = mlp_path + '/' + 'checkpoint.pth'
        self.mlp.load_state_dict(torch.load(best_mlp_path))
