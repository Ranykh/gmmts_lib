...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(vali_loader):
                ...
                prompt_emb, _ = self.mlp(prompt_emb)  # (batch, prompt_token, text_embedding_dim)
                # decoder input
    ...
    def train(self, setting):
        ...
        path = os.path.join(os.path.join(self.args.checkpoints, "model"), setting)
        if not os.path.exists(path):
            os.makedirs(path)
        mlp_path = os.path.join(os.path.join(self.args.checkpoints, "mlp"), setting)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(train_loader):
                ...
                if self.use_fullmodel:
                    ...
                    prompt_emb=prompt_embeddings 
                prompt_emb, _ = self.mlp(prompt_emb) 
                # decoder input
                dec_inp = torch.zeros_like(batch_y[:, -self.args.pred_len:, :]).float()
                dec_inp = torch.cat([batch_y[:, :self.args.label_len, :], dec_inp], dim=1).float().to(self.device)
                # encoder - decoder
    ...
    def test(self, setting, test=0, data_flag='test', save_gating_dataset=False):
        print("using models stored in self.model and self.mlp - run only after training")
        ...
        latent_num_embs = []
        latent_text_embs = []
        folder_path = './{}/results/'.format(data_flag) + setting + '/'
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(test_loader):
                ...
                if save_gating_dataset:
                    raw_inputs.append(batch_x.detach().cpu())
                    latent_num_embs.append(latent_num_emb.detach().cpu())
                    latent_text_embs.append(latent_text_emb.detach().cpu())
        ...
        print(f"preds, trues shape for data {data_flag}:", preds.shape, trues.shape)
        try:
        ...
        try:
            ...
            pass
        print(f"preds, trues shape for data {data_flag} after reshaping:", preds.shape, trues.shape)
        ...
        if save_gating_dataset:
            ...
            np.save(folder_path + 'pred.npy', preds)
            np.save(folder_path + 'true.npy', trues)
            torch.save(raw_inputs, folder_path + 'raw_inputs.pth')
            if self.prompt_weight < 1:
                torch.save(latent_num_embs, folder_path + 'latent_num_embs.pth')
            if self.prompt_weight > 0:
                torch.save(latent_text_embs, folder_path + 'latent_text_embs.pth')
